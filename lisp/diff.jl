...
(defun diff-configure-views (src-range dst-range)
  ...
  (with-view (car (window-view-list))
    ...
    (goto (pos 0 (max 0 (car src-range))))
    (center-display nil (max 1 (/ (- (cdr (view-dimensions))
				     (- (cdr src-range)
					(car src-range))) 2))))
  (with-view (nth 1 (window-view-list))
    (goto-buffer diff-dest-buffer)
    (goto (pos 0 (max 0 (car dst-range))))
    (center-display nil (max 1 (/ (- (cdr (view-dimensions))
				     (- (cdr dst-range)
					(car dst-range))) 2)))))
...
(defun diff-parse-range (pos cell)
  (let
    ...
    (if (match-start 2)
	(setq end (1- (read-from-string (expand-last-match "\\3"))))
      (setq end start))
    (setcar cell start)
    (setcdr cell end)
    (match-end)))
  ...
  (let
    ...
    (with-buffer diff-src-buffer
      (setq diff-src-extent (make-extent
			     (if (eq command 'add)
				 (if (>= (car left-range) 0)
				     (end-of-line (pos 0 (car left-range)))
				   (pos 0 0))
			       (pos 0 (car left-range)))
			     (pos 0 (1+ (cdr left-range)))
			     (list 'face (cond
					  ((eq command 'change)
					   diff-changed-face)
					  ((eq command 'delete)
    ...
    (with-buffer diff-dest-buffer
      (setq diff-dest-extent (make-extent
				  ...
				  (if (>= (car right-range) 0)
				     (end-of-line (pos 0 (car right-range)))
				   (pos 0 0))
				(pos 0 (car right-range)))
			      (pos 0 (1+ (cdr right-range)))
			      (list 'face (cond
					   ((eq command 'add)
					    diff-added-face)
					   ((eq command 'change)
