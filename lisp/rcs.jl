...
(defun rcs-call-callback ()
  ...
  (let
    ...
    (let
	((file (list-ref rcs-callback-args 0))
	 (command (list-ref rcs-callback-args 2))
	 (options (if (string-match "^[ \t\n]*$" text)
		      (list-ref rcs-callback-args 3)
		    (cons (concat (list-ref rcs-callback-args 4) text)
			  (list-ref rcs-callback-args 3))))
	 (reread-p (list-ref rcs-callback-args 5))
