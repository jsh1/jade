	  ...
	  (cons #\a (lambda (m)
		     (concat (cond
			     ...
			     (cond
			      ...
			      (t #\space))
			     (if (rm-message-get m 'filed) #\F #\space))))
	  (cons #\A (lambda (m)
		     (concat (if (rm-message-get m 'unread) #\U #\space)
			     (if (rm-message-get m 'deleted) #\D #\space)
			     (if (rm-message-get m 'replied) #\R #\space)
			     (if (rm-message-get m 'forwarded) #\Z #\space)
			     (if (rm-message-get m 'filed) #\F #\space))))
	  (cons #\* (lambda (m)
