...
(defvar rm-summary-keymap
  (bind-keys (make-sparse-keymap summary-keymap)
    ...
    "R" '(rm-command-in-folder '(rm-reply t nil current-prefix-arg))
    "f" '(rm-command-in-folder 'rm-followup)
    "F" '(rm-command-in-folder '(rm-followup t current-prefix-arg))
  ...
  `(("Movement"
    ...
    ("Dispose message"
     ...
     ("Unmark all messages" ,#'(lambda ()
				 (rm-command-with-folder 'rm-unmark-all-messages)))
     ()
